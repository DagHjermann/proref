...
# Parameters lost - these are 3 sum parameters 
params_fate4 %>% 
...
# these are lost because all stations end up as background stations
# (i.e., no station was significantly different from all stations with lower concentrations)
result_detailed %>%
  filter(
...
#### final 'params_fate' table  
```{r}

table(addNA(params_fate5$filtered_high_lessthans))
table(addNA(params_fate5$result1))
table(addNA(params_fate5$proref_paper))
params_fate_final <- params_fate5 %>% 
  mutate(
    filtered_high_lessthans = case_when(
      filtered_high_lessthans == 1 ~ 0,
      TRUE ~ 1),
    filtered_too_few_years = case_when(
      is.na(result1) & filtered_high_lessthans == 1 ~ 0,
      filtered_high_lessthans == 1 ~ 1,
      filtered_high_lessthans == 0 ~ NA),
    filtered_only_background_stations = case_when(
      filtered_high_lessthans %in% 1 &
        filtered_too_few_years %in% 1 &
          proref_paper %in% 1 ~ 1,
      filtered_high_lessthans %in% 1 &
        filtered_too_few_years %in% 1 &
          proref_paper %in% 0 ~ 0)
  ...
  select(PARAM:Substance.Group, 
         filtered_high_lessthans,
         filtered_too_few_years,
         filtered_only_background_stations)

table(addNA(params_fate_final$filtered_high_lessthans))
table(addNA(params_fate_final$filtered_too_few_years))
table(addNA(params_fate_final$filtered_only_background_stations))
...
if (save_results){
  ...
  fn3 <- "Data/54_params_fate.csv"
  readr::write_csv(params_fate_final, fn3)
  cat("'params_fate_final' saved as ", fn3, "\n")
